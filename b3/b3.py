...
class B3:
  ...
  def compute_class_precision_recall(self,L,K):
    '''
  ...
  def calc_b3(self , L , K , class_norm=False, beta=1.0):
    '''
    ...
    precision_recall , class_sizes = self.compute_class_precision_recall(L,K)
    
    # Two methods of obtaining overall precision and recall
    if(class_norm == True):
      precision = P_i.sum() / P_i.size()  
      recall    = R_i.sum() / R_i.size()
    else:
